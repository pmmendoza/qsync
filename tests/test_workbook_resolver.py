...
class TestWorkbookResolver:
    ...
    def test_slug_from_cached_survey_title(self, tmp_path):
        ...
        mock_survey.payload = {
            "result": {
                "SurveyOptions": {
                    "SurveyTitle": "Cached Survey Title"
                }
            }
        }
        
        with patch("qsync.qualtrics_client.load_cached_survey", return_value=mock_survey):
            resolver = WorkbookResolver(root=tmp_path)
            result = resolver.default_path("SV_123")
    ...
    def test_slug_fallback_to_survey_id(self, tmp_path):
        ...
        with patch("qsync.qualtrics_client.load_cached_survey", side_effect=Exception("No cache")):
            resolver = WorkbookResolver(root=tmp_path)
            result = resolver.default_path("SV_123")
    ...
    def test_csv_read_error_handled(self, tmp_path):
        ...
        with patch("qsync.qualtrics_client.load_cached_survey", side_effect=Exception()):
            resolver = WorkbookResolver(root=tmp_path)
            result = resolver.default_path("SV_123")
