...
def test_sync_items_stages_then_pushes_when_only_workbook_diffs_exist(
    ...
    from qsync.sync_core import PreviewChange

    # Ensure pending files are written under tmp_path, not the real repo.
    monkeypatch.setattr(
        "qsync.pending_stage.resolve_root", lambda required=False: tmp_path
    )
    ...
    monkeypatch.setattr(
        "qsync.workbook_resolver.WorkbookResolver.resolve", lambda self, survey_id: wb
    )

    # Pretend there is an unstaged workbook diff.
    def fake_preview(survey_id, xlsx_path, **kwargs):
        return [PreviewChange(kind="question", qid="QID1", old_html="a", new_html="b")]
    ...
    monkeypatch.setattr(
        "qsync.dimensions.items.load_cached_survey",
        lambda *a, **k: type(
            "S",
            (),
    ...
    monkeypatch.setattr(
        "qsync.dimensions.items._collect_embedded_data_changes", lambda *a, **k: []
    )
    ...
    monkeypatch.setattr(
        "qsync.dimensions.items.push_staged_changes", fake_push_staged_changes
    )
