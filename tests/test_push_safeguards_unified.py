...
class TestEnforcePushSafeguards:
    ...
    def test_unlocked_survey_no_responses_passes(self, mock_push_context):
        ...
        with (
            ...
            patch(
                "qsync.push_safeguards.load_push_context",
                return_value=mock_push_context,
            ),
        ):
    ...
    def test_locked_survey_blocks_without_allow_locked(self, mock_push_context):
        ...
        with (
            patch(
                "qsync.push_safeguards.ensure_unlocked",
                side_effect=SurveyLockedError("Survey is locked"),
            ),
            patch(
                "qsync.push_safeguards.load_push_context",
                return_value=mock_push_context,
    ...
    def test_locked_survey_passes_with_allow_locked(self, mock_push_context):
        ...
        with patch(
            "qsync.push_safeguards.load_push_context", return_value=mock_push_context
        ):
    ...
    def test_live_responses_warns_with_force_live_interactive_abort(
        self, mock_push_context, capsys
    ):
    ...
    def test_preview_only_suppresses_warning_with_force_preview(
        self, mock_push_context, capsys
    ):
    ...
    def test_dimension_specific_prompts(self, mock_push_context):
        ...
        for dimension, expected_prompt in dimensions_and_prompts:
            with (
                ...
                patch(
                    "qsync.push_safeguards._prompt_confirmation", return_value=True
                ) as mock_prompt,
            ):
