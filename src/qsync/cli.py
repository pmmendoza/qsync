...
def _main_impl(argv: Optional[list[str]] = None) -> None:
    ...
    if args.command == "doctor":
        ...
        inventory_resolved = (
            ...
            if inventory_csv.exists()
                ...
                legacy_inventory_csv if legacy_inventory_csv.exists() else inventory_csv
            )
        )
        ...
        elif inventory_resolved == legacy_inventory_csv and not inventory_csv.exists():
            warnings.append(
        ...
        mapping_workspace_surveys = root / "surveys" / "qualtrics_api_key_mapping.csv"
            ...
            try:
                ...
                if datacenter and base_url:
                    ...
                    if re.fullmatch(r"[a-z]{2,5}\d+", left_label or ""):
                        datacenter_mismatch = left_label != datacenter.strip().lower()
                    else:
    ...
    if args.command == "eos":
        ...
        if args.eos_command == "references":
            ...
            header(
                "[qsync:eos]", f"References for {args.library_id}/{args.message_id}:"
            )
            if not contexts:
                info(
                    "[qsync:eos]", "No references found in cached surveys (local scan)."
                )
                return
            for ctx in contexts:
        ...
        if args.eos_command in ("stage", "apply"):
            ...
            if (
                ...
                info("[qsync:eos]", "No local EOS changes detected; nothing staged.")
                return
            success(
                "[qsync:eos]",
    ...
    if args.command == "js":
        ...
        if args.js_command in ("stage", "apply"):
            ...
            if args.js_command == "apply":
                ...
                warn("[DEPRECATION] 'js apply' is deprecated. Use 'js stage' instead.")
        ...
        if args.js_command == "push":
            ...
            def _filter_entries(entries: list[dict[str, str]]) -> list[dict[str, str]]:
                filtered = list(entries)
                if include_qids:
                    filtered = [e for e in filtered if e.get("qid") in include_qids]
                if include_js:
                    filtered = [e for e in filtered if e.get("js_file") in include_js]
                return filtered
            ...
            if not args.yes:
                try:
                    ...
                    resp = input("Push these QuestionJS blocks? [Y/n] ").strip().lower()
                    if resp and resp not in {"y", "yes"}:
                        warn("[qsync:js]", "Aborted.")
                        return
            try:
    ...
    if args.command == "sync":
        ...
        if getattr(args, "scope", None):
            try:
                scope = ScopeFilter.parse(
                    args.scope,
                    survey_id=args.survey_id if hasattr(args, "survey_id") else None,
                )
            except Exception as e:
        ...
        if getattr(args, "dimensions", None):
            ...
            if invalid:
                ...
                error(
                    "[qsync:sync]", f"Valid dimensions: {', '.join(sorted(valid_dims))}"
                )
                raise SystemExit(1)
    ...
    if args.command == "items":
        ...
        if args.items_command == "preview":
            ...
            def _update_cache() -> None:
                refresh_survey_cache(args.survey_id)
                info("[qsync:items]", "Refreshed cached survey definition from API.")
            ...
            if not changes:
                info("[qsync:items]", "No differences between Excel and cached survey.")
                return
    ...
    if args.command == "preview":
        ...
        if not changes:
            info("[qsync:preview]", "No differences between Excel and cached survey.")
            return
    ...
    if args.command == "push":
        ...
        if not args.yes:
            ...
            if embedded_count and not change_count:
                ...
                prompt_suffix = (
                    f"{change_count} question(s) and {embedded_count} embedded field(s)"
                )
            else:
        ...
        if change_count and embedded_count:
            ...
            success("[qsync:push]", f"Uploaded {change_count} question(s): {qid_list}.")
        return
